...
authRouter.post('/register', registerValidationRules(), validadeRequest, async (req, res) => {
    try {
...
authRouter.post('/refresh-token', limiter, async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: user._id }, process.env.SECRET_KEY, { expiresIn: '2h'});
        const newRefreshToken = jwt.sign({ userId: user._id}, process.env.REFRESH_SECRET, { expiresIn: '7d'});
